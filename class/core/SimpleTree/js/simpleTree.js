  ...
  */
  ...
  SimpleTree.prototype.getXmlData = function(){
      var xmldom = null;
      var parser = null;
      ...
      if(window.ActiveXObject){
          xmldom.async = true;
          xmldom.loadXML(this.dataStr);
      }else if(window.DOMParser){
          xmldom = parser.parseFromString(this.dataStr, "text/xml");
          this.addProperty();
      }
  ...
  SimpleTree.prototype.GiveTreeNodeContent = function(node,pNode){
      ...
      if(depth == "1"){
          var imgHtml = node.childNodes.length != 0 ? "<img _isRoot=\"1\" nodeId=\""+id+"\" src=\"" + this.iconPath + (node.getAttribute("_opened") == "true" ? "root_expand.gif" : "root_contract.gif") + "\"/>" : "<img src=\"" + this.iconPath + "root_leaf.gif\"/>";
          var imgDiv = this.$C("div",{className:"treeNodeIconRootDiv",innerHTML:imgHtml});
          ...
          var imgHtml = node.childNodes.length != 0 ? "<img _isRoot=\"0\" nodeId=\""+id+"\" src=\"" + this.iconPath + (node.getAttribute("_opened") == "true" ? "expand.gif" : "contract.gif") + "\"/>" : "<img src=\"" + this.iconPath + "leaf.gif\"/>";
      ...
      if(node.getAttribute("_opened") == "false"){
        this.closedList.push([imgDiv, node]);
      }
  }
  ...
  SimpleTree.prototype.initTreeStatus = function(){
    for(var i = 0 , iLen = this.closedList.length ; i < iLen ; i ++){
      var item = this.closedList[i];
      item[1].setAttribute("_opened", "true");
      this.resetLeaf(item[0]);
    }
  }
