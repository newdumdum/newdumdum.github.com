...
//			alert("load: "+ filename+"\ncaller:"+this.init.caller);
  ...
  var init = function(){
    ...
    for(var i=0,l=Config.methods.length; i<l; i++){
		result.Store.prototype[Config.methods[i]] = empty;
	};
	...
	if(Config.option.backend){
		...
		if(Backends[b] && Backends[b].test()){
			result.type = b;
			result.size = Backends[b].size;
			for(key in Backends[b].methods){
			  result.Store.prototype[key] = Backends[b].methods[key];
			};
		}else{
			// 参数backend有误情况
			for(var i=0,l=Config.searchList.length; !result.type && i<l; i++){
			  var b = Backends[Config.searchList[i]];

			  if(b.test()){
				result.type = Config.searchList[i];
				result.size = b.size;

				// 用实例方法扩展构造函数
				for(key in b.methods){
				  result.Store.prototype[key] = b.methods[key];
				};
			  };
			};
		};
  ...
  var result = {
    ...
    Store: function(name, option){
	  ...
	  Config.option.expires = Config.option.expires || 365 * 2;
	...
	set: function(key, val, fn, option){
		if(!_storage){
			_storage = new ace.more.storage.Store("fastmulan", {});
		}
		_storage.set(key, val, fn, option);
	},
  ...
  ace.lang.module("more.storage", result, ace);
})();
